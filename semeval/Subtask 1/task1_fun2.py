...
# ============================================================
SYNONYM_DICT = {
    "Action": ["act", "perform", "execute", "implement", "do", "carry out", "conduct"],
    "Actor": ["perpetrator", "agent", "instigator", "actor", "culprit", "mastermind"],
    "Effect": ["outcome", "result", "consequence", "impact", "influence", "ramification"],
    "Evidence": ["proof", "indication", "clue", "sign", "trace", "testimony", "confirmation"],
    "Victim": ["target", "sufferer", "casualty", "scapegoat", "dupe", "prey", "innocent"]
}

def synonym_augmentation(text, marker_type):
    words = text.split()
    augmented_text = words[:]
    for i, word in enumerate(words):
        # 替换与 marker_type 相关的词汇
        if word.lower() in SYNONYM_DICT.get(marker_type, []):
            synonym = random.choice(SYNONYM_DICT[marker_type])
            augmented_text[i] = synonym
    return ' '.join(augmented_text)


# ============================================================
# 4. Tokenize + 标签对齐（含 PAD → -100）
# ============================================================
...
# ============================================================
# 5. WeightedLossTrainer（兼容新版 Trainer）
# ============================================================
class WeightedLossTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        weight = torch.tensor([1.0, 5.0]).to(logits.device)
...
if __name__ == "__main__":
    ...
    # ================================
    # 简单的数据增强（根据 marker 类型替换同义词）
    # ================================
    augmented_data = []
    for item in dataset_split["train"]:
        augmented_data.append(item)  # 原始数据
        for marker_type in marker_types_to_train:
            augmented_text = synonym_augmentation(item["text"], marker_type)  # 数据增强（同义词替换）
            augmented_item = item.copy()
            augmented_item["text"] = augmented_text
            augmented_data.append(augmented_item)

    augmented_dataset = Dataset.from_list(augmented_data)
    augmented_dataset_split = augmented_dataset.train_test_split(test_size=0.1, seed=42)
    ...
    for marker_type in marker_types_to_train:
        ...
        # tokenize
        tokenized_dataset = augmented_dataset_split.map(
