...
train = pd.read_csv(r"D:\360Downloads\labeledTrainData.tsv", header=0,
                    delimiter="\t", quoting=3)
test = pd.read_csv(r"D:\360Downloads\testData.tsv", header=0,
                   delimiter="\t", quoting=3)
unlabeled_train = pd.read_csv(r"D:\360Downloads\unlabeledTrainData.tsv", header=0,
                              delimiter="\t", quoting=3)
...
if __name__ == '__main__':
    ...
    wvmodel_file = (r"D:\360Downloads\glove.840B.300d.txt")
    wvmodel = KeyedVectors.load_word2vec_format(wvmodel_file,binary=False,no_header=True)
    """ 
    ...
    # 定义绝对路径
    base_dir = r'D:\Pycharm\IMDB\.venv'
    pickle_dir = os.path.join(base_dir, 'pickle')
    pickle_file = os.path.join(pickle_dir, 'imdb_glove.pickle3')

    # 确保目录存在 - 使用 exist_ok=True 避免目录已存在的错误
    os.makedirs(pickle_dir, exist_ok=True)

    print(f"确保目录存在: {pickle_dir}")
    print(f"目录是否存在: {os.path.exists(pickle_dir)}")

    # 保存数据
    try:
        with open(pickle_file, 'wb') as f:
            pickle.dump(imdb_glove_data, f)
        print(f"✓ 文件成功保存到: {pickle_file}")
