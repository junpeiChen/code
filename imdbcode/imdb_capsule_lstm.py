...
class Capsule(nn.Module):
    def __init__(self, num_hiddens, bidirectional, num_capsule=5, dim_capsule=5, routings=4, **kwargs):
        ...
        if self.bidirectional:
            self.W = nn.Parameter(
                nn.init.xavier_normal_(torch.empty(1, num_hiddens * 2, self.num_capsule * self.dim_capsule)))
        else:
            self.W = nn.Parameter(
                nn.init.xavier_normal_(torch.empty(1, num_hiddens, self.num_capsule * self.dim_capsule)))
...
class SentimentNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        capsule = self.capsule(states)  # (batch_size, num_capsule, dim_capsule)

        # 修正解码器的输入处理
        if self.bidirectional:
            # 将capsule输出展平
            encoding = capsule.view(capsule.size(0), -1)  # (batch_size, num_capsule * dim_capsule)
            # 确保线性层输入维度匹配
            if encoding.size(1) != self.num_hiddens * 4:
                # 如果需要，添加一个适配层
                encoding = nn.Linear(encoding.size(1), self.num_hiddens * 4).to(encoding.device)(encoding)
        else:
            encoding = capsule.view(capsule.size(0), -1)
            if encoding.size(1) != self.num_hiddens * 2:
                encoding = nn.Linear(encoding.size(1), self.num_hiddens * 2).to(encoding.device)(encoding)
