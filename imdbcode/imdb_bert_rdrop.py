...
# 读取数据
train = pd.read_csv(r"/root/autodl-tmp/labeledTrainData.tsv", header=0, delimiter="\t", quoting=3)
test = pd.read_csv(r"/root/autodl-tmp/testData.tsv", header=0, delimiter="\t", quoting=3)

MODEL_PATH = r"/root/autodl-tmp/bert-base-uncased"
...
class BertScratch(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None):
        ...
        if self.training and labels is not None:
            ...
            ce_loss1 = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            ce_loss2 = loss_fct(kl_logits.view(-1, self.num_labels), labels.view(-1))
            kl_loss = (KL(logits, kl_logits, "batchmean") + KL(kl_logits, logits, "batchmean")) / 2.0

            # 总损失 = 两个CE损失的平均 + KL损失
            total_loss = (ce_loss1 + ce_loss2) / 2.0 + kl_loss
        else:
            # 在评估或预测时，只计算一次前向传播
...
if __name__ == '__main__':
    ...
    model = BertScratch.from_pretrained(MODEL_PATH)
